...
//                System.out.print("serial busy");
        ...
        //---------------Sleep----------------
        try{  Thread.sleep(this.interval * 2);  } 
        catch (InterruptedException ex){ _serial_status = 0; return false;  }
        //------------------------------------
...
//            System.out.println(length);
            System.out.println("set Min Max reply length = " + length);
...
//            int length = this._in.read(buffer);
    ...
    public boolean setMinMax(int[] min, int[] max){
        try{
            byte[] mi = new byte[min.length * 2];
            byte[] ma = new byte[max.length * 2];
            for (int i = 0; i < min.length; i++) {
                mi[2*i] = (byte)((min[i] >> 8) & 0xff);
                mi[2*i + 1] = (byte)(min[i] & 0xff);
            }
            for (int i = 0; i < max.length; i++) {
                ma[2*i] = (byte)((max[i] >> 8) & 0xff);
                ma[2*i + 1] = (byte)(max[i] & 0xff);
            }
            setMinMax(mi, ma);
        }catch(Exception ex){
            this._error = "getDistance | Read | Exception Error | " + ex.getMessage();
...
//            int length = this._in.read(buffer);
        ...
        for (int i = 0; i < max_distance.length; i++) {
            if(result[i] > max_distance[i])
                max_distance[i] = result[i];
            if(min_distance[i] == 0)
                min_distance[i] = mn[i];
//            if(mn[i] < min_distance[i])
//                min_distance[i] = mn[i];
        }
        this.buffer = result;
        return result;
    } 
    //5 edit Detected Delay
    public boolean editDetectedDelay(int start_delay, int end_delay){
        //--------------Send--------------
        try{
            //wait if serial is busy
            while(_serial_status != 0){
//                System.out.print("serial busy");
            ...
            data[0] = (byte)'{';     
            data[1] = 5; //command id 
            data[2] = (byte)((start_delay >> 8) & 0xff);
            data[3] = (byte)(start_delay & 0xff);
            data[4] = (byte)((end_delay >> 8) & 0xff);
            data[5] = (byte)(end_delay & 0xff);

            int crc8 = alisa.CRC.crc8(data, 0, 6);
            data[6] = (byte)(crc8); 
            data[7] = (byte)'}';
            this._out.write(data);
            System.out.println("Sent");
            ...
            this._error = "editDetectedDelay | Write | Exception Error | " + ex.getMessage();
            this._connected = false;
            this._reconnect();
            _serial_status = 0;
            return false;
...
//            int length = this._in.read(buffer);
            ...
            System.out.println("Edite Delay length = " + length);
            if (length >= 5 && buffer[0] == (byte)'{'  && buffer[1] == 5 && buffer[4] == (byte)'}'){ // slave_id & function code                            
                if (alisa.CRC.crc8(buffer, 0, 4) == 0){ // check CRC8
                   System.out.println("Received!!!!");
                    for (int i=0; i<length; i++){
                        System.out.print((buffer[i] & 0xFF) + " ");
                   ...
                   System.out.println();
                    ...
                    this._error = "editDetectedDelay | Read | CRC Error";
                    this._connected = false;
                    _serial_status = 0;
                    return false;
                }
                ...
                this._error = "editDetectedDelay | Read | Data Error";
                this._connected = false;
                _serial_status = 0;
                return false;
            }               
            ...
            this._error = "editDetectedDelay | Read | Exception Error | " + ex.getMessage();
            this.distance = -3; 
            this._connected = false;
            this._reconnect();
            _serial_status = 0;
    ...
    int start_detected_delay = 0;
    int end_detected_delay = 0;
    //6 get Detected Delay
    public boolean getDetectedDelay(){
        //--------------Send--------------
        try{
            //wait if serial is busy
            while(_serial_status != 0){
//                System.out.print("serial busy");
            ...
            this._error = "getDetectedDelay | Write | Exception Error | " + ex.getMessage();
            this._connected = false;
            this._reconnect();
            _serial_status = 0;
            return false;
...
//            int length = this._in.read(buffer);
            ...
            System.out.println("Get Delay length = " + length);
            for (int i = 0; i < length; i++) {
                System.out.print(buffer[i] + " ");
            }
            if (length >= 8 && buffer[0] == (byte)'{'  && buffer[1] == 6 && buffer[7] == (byte)'}'){ // slave_id & function code                            
                if (alisa.CRC.crc8(buffer, 0, 7) == 0){ // check CRC8
                    System.out.println("Received!!!!");
                    start_detected_delay = (int)((buffer[2] & 0xff) << 8) + (int)(buffer[3] & 0xff);
                    end_detected_delay = (int)((buffer[4] & 0xff) << 8) + (int)(buffer[5] & 0xff);
                    System.out.println();
                } 
                else{
                    this._error = "editDetectedDelay | Read | CRC Error";
    ...
    //----------------------------------------------------------
    //new
    public boolean auto_assign_minmax(int[] distance, int[] max_distance, int[] min_distance){
        int[] min = this.getMin();
        int[] max = this.getMax();
        for (int i = 0; i < max_distance.length; i++) {
//            int min = (int)((Min[2*i] << 8) & 0xff) + (int)(Min[2*i + 1] & 0xff);  
//            int max = (int)((Max[2*i] << 8) & 0xff) + (int)(Max[2*i + 1] & 0xff);  
            if(distance[i] > max[i] && distance[i] < max_distance[i] - 20){
                max[i] = distance[i];
            }
            if(min[i] == 0 && distance[i] < max_distance[i] - 20){
                if(distance[i] <= max[i])
                    min[i] = distance[i];
            }else if(distance[i] < min[i] && distance[i] > 0 + 10){
//                if(distance[i] <= max[i])
                    min[i] = distance[i];
            }
        }
