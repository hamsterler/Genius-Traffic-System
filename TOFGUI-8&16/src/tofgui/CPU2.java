...
public class CPU2 extends Thread
{
    ...
    public CPU2(String port)
    {
        ...
        this.getVersion();
        _distance = new int[this.line_num];
    }
    private int[] _distance;
    @Override
    public void run(){
        try {
            _distance = getDistanceInt();
            Thread.sleep(10);
        } catch (InterruptedException ex) {
            Logger.getLogger(CPU2.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public boolean getData(){
        ...
        try{
            ...
            if (length >= line_num * 4 + 4 && buffer[0] == (byte)'{'  && buffer[1] == CPU2.GetData && buffer[line_num * 4 + 3] == (byte)'}'){ // slave_id & function code                         
    ...
    public synchronized int[] getDistanceInt(){              
        ...
        try{
            ...
            if (length >= line_num * 2 + 6 && buffer[0] == (byte)'{'  && buffer[1] == CPU2.GetDistance && buffer[line_num * 2 + 5] == (byte)'}'){ // slave_id & function code                            
