...
//            Serial.print(_distance[i]);
  ...
  // ------ User Interface (Serial) ---------
    ...
    if (length_serial >= 68) // write EEPROM
    {
      if (buf[0] == (byte)'{'  && buf[1] == 1 && buf[67] == (byte)'}') 
      {                            
            if (_crc8(buf, 0, 67) == 0) // check CRC8
            {
                //write EEPROM
                for(int i=0; i<16; i++)
                {
                  ...
                  // Send    
                   byte b[68];
                    ...
                    for(int i=0; i<16; i++)
                    {
                        ...
                        b[2*i + 34] = EEPROM.read(2*i + 32);  // max[hi]
                        b[2*i + 35] = EEPROM.read(2*i + 33);  // max[lo]
                    }
                    
                    int crc8 = _crc8(b, 0, 66);
                    b[66] = (byte)(crc8); 
